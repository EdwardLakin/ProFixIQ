...
6. **Legitimate portal customers cannot use the portal.**
   `customer_portal_invites` has RLS enabled
   (`20260705000000_public_schema_baseline.sql:5427`) but **no SELECT policy
   exists in any migration**. The sign-in guard reads it with the service-role
   client (`app/api/auth/sign-in/route.ts:192-202`), while
   `requirePortalCustomerAccess` (`features/portal/server/portalAuth.ts:71-108`)
   re-queries it with the end-user client and therefore returns zero rows
   unconditionally. This breaks portal entry **and**
   `app/api/portal/payments/checkout/route.ts:44`, so customer payment checkout
   is blocked. Single highest-value fix in this document.
