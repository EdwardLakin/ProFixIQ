...
| `20260806181742_reconcile_migration_alias_effects.sql`        | Adds the one effect missing from the non-identical invoice hardening alias and asserts the final agent-bridge state.                         |
...
| `20260805202600`   | `20260805204119`, `20260805214807` | Composite executions migrated the secret into `integrations` and dropped the credential table. Current RLS is equivalent and narrower (`authenticated`, not `public`). |
...
2. Mark the 22 effect-equivalent repository versions as `applied` with the
...
3. Mark all 25 execution-time aliases as `reverted`, including the
...
4. Mark the seven production-only rows classified above as `reverted`.
...
5. Run `supabase migration list --linked` and save the result. Leave
   `20260804031005` and `20260806021000` unapplied so their SQL executes.
6. Run `supabase db push --linked --include-all --dry-run`. The only pending
   versions must be `20260804031005`, `20260806021000`, and the four repair
   migrations listed above.
7. Run `supabase db push --linked --include-all` once.
8. Re-run migration-list comparison, generated linked types, runtime
