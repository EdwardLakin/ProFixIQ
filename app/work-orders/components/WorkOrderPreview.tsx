// app/work-orders/components/WorkOrderPreview.tsx
...
export default async function WorkOrderPreview({ woId }: Props) {
  if (!woId) {
    return (
  ...
  const { data: wo, error: woErr } = await supabase
    .from("work_orders")
    .select(
      "id,status,type,created_at,updated_at,notes,customer_id,vehicle_id,invoice_total,labor_total,parts_total,quote_url,invoice_url"
    )
    .eq("id", woId)
    .single();
  ...
  if (woErr || !wo) {
    return (
      <div className="text-red-400 text-sm">
        Failed to load work order: {woErr?.message || "not found"}
      </div>
    );
  }
  ...
  let vehicle: Pick<
    Tables<"vehicles">,
    "year" | "make" | "model" | "license_plate" | "vin" | "color" | "mileage" | "unit_number"
  > | null = null;
  ...
  // ── Lines (safe default to []) ────────────────────────────────────────────────
  const { data: linesRaw } = await supabase
    .from("work_order_lines")
    .select(
      "id, description, job_type, labor_time, notes, complaint, cause, correction, status, created_at, updated_at"
    )
  ...
  // Fix '||' with '??' precedence by grouping properly
  const customerName =
    customer?.name ??
  ...
  return (
    <div
      className="rounded-lg border bg-neutral-950 p-5 shadow-xl"
      style={{ borderColor: "#f97316" /* orange-500 */ }}
    >
      ...
      >
        ...
        <section className="space-y-1">
          ...
          <div className="text-neutral-300">
            Color: {vehicle?.color || "—"} · Mileage: {vehicle?.mileage ?? "—"} · Unit:{" "}
            {vehicle?.unit_number || "—"}
          </div>
        </section>
      </div>
        ...
        <div className="mt-4 flex gap-2">
          {wo.quote_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-xs underline text-orange-400 hover:text-orange-300"
            >
              Open Quote
            </a>
          ) : null}
