...
const presetOptions: WOItem[] = [
  ...
  { type: "inspection", description: "Pre-Purchase Inspection", selected: false },
...
export default function CustomerWorkOrderPage() {
  ...
  const submitWorkOrder = async () => {
    ...
    if (!selectedItems.length || !date || !time) return alert("Please complete all fields.");
  ...
  return (
    <div className="p-4 space-y-6">
      <h1 className="text-2xl font-bold text-orange-400">Build Your Work Order</h1>
        ...
        {items.map((item, index) => (
          <button
            ...
            className={`w-full text-left px-4 py-2 rounded-md border ${
              item.selected ? "bg-green-500 text-black" : "bg-white/10 text-white"
            }`}
