...
export default async function HistoryDetailPage({ params }: { params: Promise<{ id: string }> }): Promise<JSX.Element> {
  ...
  return <div className="min-h-[calc(100vh-4rem)] bg-[var(--theme-gradient-panel)] px-4 py-6 text-[color:var(--theme-text-primary)]"><div className="mx-auto max-w-5xl space-y-4"><Link href="/work-orders/history" className="inline-flex items-center gap-2 rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-panel)] px-3 py-1.5 text-xs uppercase tracking-[0.16em] text-[color:var(--theme-text-primary)]">← Back to History</Link><section className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-page)] p-4"><h1 className="font-mono text-xl text-sky-200">History #{historyShortId(row.id)}</h1></section><section className="rounded-xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-panel)] p-3 text-sm"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3"><div>Work Order: {row.work_order_number ?? p.workOrderLabel ?? "—"}</div><div>Invoice: {row.invoice_number ?? p.invoiceLabel ?? "—"}</div><div>Status: {row.historical_status ?? "—"}</div><div>Opened: {row.opened_at ? format(new Date(row.opened_at), "PPpp") : serviceDate}</div><div>Closed: {row.closed_at ? format(new Date(row.closed_at), "PPpp") : "—"}</div><div>Priority: {row.priority ?? "—"}</div><div>Advisor: {row.advisor_name ?? "—"}</div><div>Tech: {row.assigned_tech_name ?? "—"}</div><div>Odometer: {row.odometer ?? "—"}</div><div>Labor Hours: {row.labor_hours ?? "—"}</div><div>Labor: {formatMoneyLike(String(row.labor_sale ?? "")) ?? "—"}</div><div>Parts: {formatMoneyLike(String(row.parts_sale ?? "")) ?? "—"}</div><div>Tax: {formatMoneyLike(String(row.tax ?? "")) ?? "—"}</div><div>Total: {formatMoneyLike(String(row.total ?? "")) ?? formatMoneyLike(p.totalLabel) ?? "—"}</div><div>Approval: {row.approval_state ?? "—"}</div><div>Payment: {row.payment_state ?? "—"}</div></div></section></div></div>;
}
