...
import { requireAuthedUser } from "@/features/portal/server/portalAuth";
...
export default async function InternalWorkOrderViewerPage({
  params,
}: {
      ...
      <WorkOrderViewer
        ...
        customer={
          customer
            ? {
                ...customer,
                phone: pickCustomerPhone(customer) ?? customer.phone ?? null,
              }
            : undefined
        }
        shop={
        ...
        title="Work order"
        subtitle="Read-only view (internal)."
        invoicePdfUrl={wo.invoice_pdf_url ?? null}
      />
    );
  } catch (e) {
    // eslint-disable-next-line no-console
    console.error("[internal work order viewer] failed:", e);
    redirect("/");
  }
}
