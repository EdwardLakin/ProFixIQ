...
export default async function PortalInvoicePage({
  ...
  try {
    ...
    // -------------------------------------------------------------------
    // Load latest invoice row for this work order (SOURCE OF TRUTH for totals/currency)
    // -------------------------------------------------------------------
  ...
  const total = invoice?.total != null ? safeNumber(invoice.total) : invoiceTotalFallback;

  const subtotal =
    invoice?.subtotal != null ? safeNumber(invoice.subtotal) : undefined;

  const laborCost =
    invoice?.labor_cost != null ? safeNumber(invoice.labor_cost) : undefined;

  const partsCost =
    invoice?.parts_cost != null ? safeNumber(invoice.parts_cost) : undefined;

  const discountTotal =
    invoice?.discount_total != null ? safeNumber(invoice.discount_total) : undefined;

  const taxTotal =
    invoice?.tax_total != null ? safeNumber(invoice.tax_total) : undefined;
    ...
    >
      <div className="mx-auto flex min-h-screen max-w-3xl flex-col justify-center py-10">
        ...
        >
            ...
            <div className="mb-6">
              <a
                ...
                className="
                  inline-flex items-center gap-2 rounded-full border
                  border-[color:var(--metal-border-soft,#1f2937)]
                  bg-black/70 px-4 py-2 text-xs font-semibold
                  uppercase tracking-[0.18em] text-neutral-100
                  hover:bg-black/80
                "
              >
          ...
          {/* Totals breakdown (invoice row when available) */}
