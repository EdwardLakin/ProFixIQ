...
export default async function PropertyMemberRequestsPage() {
  ...
  const { data: requests } = await supabase.from("property_maintenance_requests").select("id,shop_id,property_id,unit_id,asset_id,title,status,severity,created_at").in("shop_id", shopIds).order("created_at", { ascending: false }).limit(100);
