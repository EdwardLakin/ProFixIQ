...
const inputClass =
  "w-full rounded-xl border border-[color:var(--theme-input-border)] bg-[color:var(--theme-input-bg)] px-3.5 py-3 text-sm text-[color:var(--theme-input-text)] outline-none focus:border-[var(--accent-copper)] focus:ring-4 focus:ring-[color:color-mix(in_srgb,var(--accent-copper)_16%,transparent)]";
...
export default function FleetInvitePage() {
    ...
    void (async () => {
      try {
        ...
        if (code) {
          ...
          if (exchangeError)
            throw new Error("This invitation could not be verified.");
        }
        const response = await fetch(
          `/api/portal/fleet/invites/preview?token=${encodeURIComponent(token)}`,
          { cache: "no-store" },
        );
        ...
        if (!response.ok || !payload?.invite)
          throw new Error(
            payload?.error || "This fleet invitation is unavailable.",
          );
        ...
        if (!cancelled)
          setError(
            value instanceof Error
              ? value.message
              : "This fleet invitation is unavailable.",
          );
      } finally {
  ...
  async function activate(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!legalAccepted)
      return setError(
        "Review and accept the portal terms before activating access.",
      );
    setSubmitting(true);
    try {
      ...
      if (!userData.user)
        throw new Error(
          "Open the one-time invitation email on this device before activating access.",
        );
      ...
      if (passwordError)
        throw new Error(
          "Your password could not be saved. Try a different password.",
        );
      ...
      if (!response.ok)
        throw new Error(
          payload?.error || "Fleet access could not be activated.",
        );
      ...
      setError(
        value instanceof Error
          ? value.message
          : "Fleet access could not be activated.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-8 text-sm text-[color:var(--theme-text-secondary)]">
          <Loader2 className="h-4 w-4 animate-spin" /> Verifying invitation…
        </div>
      ) : null}
      {error ? (
        <div className="mt-5">
        ...
        <>
          <div className="mt-5 divide-y divide-[color:var(--theme-border-soft)] rounded-xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] px-4 text-sm">
            <div className="flex items-center justify-between gap-3 py-3">
              ...
              <strong>{invite.fleetName}</strong>
            </div>
            <div className="flex items-center justify-between gap-3 py-3">
          ...
          <form onSubmit={activate} className="mt-5 space-y-4">
            ...
            <label className="flex items-start gap-3 rounded-xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] p-3.5 text-xs leading-5 text-[color:var(--theme-text-secondary)]">
              ...
              />
              <span>
                I have authority to use this fleet account, agree to the{" "}
                <Link
            ...
            >
              ...
              {submitting ? "Activating…" : "Create account & continue"}
            </button>
          </form>
