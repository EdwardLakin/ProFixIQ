...
// Match what /api/portal/bookings returns (shop calendar / admin list)
type PortalBooking = {
  id: string;
...
export default function PortalCustomerAppointmentsPage() {
  ...
  // 2) Resolve the customer's shop slug (so we can reuse /api/portal/bookings)
  useEffect(() => {
    let cancelled = false;
  ...
  // 3) Fetch bookings using the EXISTING shop appointments endpoint,
  // then filter client-side to THIS customer only.
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Wide window so "My appointments" works without needing a new endpoint.
        // Past 180 days + next 365 days
        const now = new Date();
  ...
  async function cancelBooking(id: string) {
    ...
    try {
      // Reuse existing endpoint family (no new files)
    ...
    <div className="mx-auto w-full max-w-xl space-y-5 text-white">
      ...
      <header className="space-y-1">
        ...
        <p className="text-xs text-neutral-400">
          View upcoming bookings and past visits.
        </p>
      </header>
      ...
      <p className="text-[0.75rem] text-neutral-500">
        Need to change a time? Cancel and book a new slot.
      </p>
    </div>
  );
}