...
export default function CreateUserPage() {
  ...
  shop_name: null,
    ...
    if (profileError) {
      ...
      setData({ full_name: '', email: '', phone: '', tempPassword: '', role: 'mechanic', plan: 'free' });
      fetchUsers();
    }
  };
  ...
  const handleUpdate = async (id: string) => {
    ...
    const { error } = await supabase.from('profiles').update(editFields).eq('id', id);
    if (!error) {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto text-white font-blackops">
      ...
      <form onSubmit={handleCreateUser} className="space-y-3 mb-8">
        <input type="text" placeholder="Full Name" value={data.full_name}
        ...
        <input type="text" placeholder="Phone" value={data.phone}
          ...
          className="w-full p-2 rounded bg-gray-800 border border-orange-500"
        />
        <input type="email" required placeholder="User Email" value={data.email}
          onChange={(e) => setData({ ...data, email: e.target.value })}
          className="w-full p-2 rounded bg-gray-800 border border-orange-500"
        />
        <input type="text" required placeholder="Temporary Password" value={data.tempPassword}
      ...
      {filteredUsers.length === 0 ? (
        ...
        <>
                ...
                {editingUserId === user.id ? (
                  <div className="flex flex-col md:flex-row gap-2 w-full md:items-center">
                    ...
                    <div className="flex gap-2 ml-auto">
                      <button onClick={() => handleUpdate(user.id)} className="bg-green-600 text-white px-3 py-1 rounded">
                        Save
                      </button>
                      <button onClick={() => { setEditingUserId(null); setEditFields(null); }}
                  ...
                  <div className="flex flex-col md:flex-row justify-between w-full md:items-center">
                    <p>
                      <span className="text-orange-400">{user.full_name}</span> – {user.email} ({user.role})
                    </p>
          ...
          <div className="flex justify-center gap-4 mt-6">
            {currentPage > 1 && (
              <button onClick={() => setCurrentPage((p) => p - 1)} className="text-orange-500 hover:underline">
                ← Prev
              </button>
            )}
