...
export async function GET(request: Request) {
  ...
  const customerMap = new Map<string, { id: string; name: string | null; phone: string | null; phone_number: string | null }>();
  for (const row of [...(byName.data ?? []), ...(byPhone.data ?? [])]) customerMap.set(row.id, row);
  ...
  return NextResponse.json({
    customers: customers.map((customer) => ({
      ...
      vehicles: (vehiclesResult.data ?? []).filter((vehicle) => vehicle.customer_id === customer.id),
    })),
  });
}
...
export async function POST(request: Request) {
  ...
  if (!body) return NextResponse.json({ error: "Invalid intake payload." }, { status: 400 });

  const operationKey = body.operationKey?.trim() || request.headers.get("idempotency-key")?.trim() || "";
  const startsAt = body.startsAt ? new Date(body.startsAt) : null;
  if (!operationKey) return NextResponse.json({ error: "An idempotency key is required." }, { status: 400 });
  if (!startsAt || Number.isNaN(startsAt.getTime())) {
    return NextResponse.json({ error: "A valid arrival time is required." }, { status: 400 });
  }

  const durationMinutes = Math.trunc(Number(body.durationMinutes ?? 60));
  if (durationMinutes < 5 || durationMinutes > 720) {
    return NextResponse.json({ error: "Visit length must be 5–720 minutes." }, { status: 400 });
  }
  const quotedPrice = body.quotedPrice == null || body.quotedPrice === ("" as never)
    ? null
    : Number(body.quotedPrice);
  if (quotedPrice != null && (!Number.isFinite(quotedPrice) || quotedPrice < 0)) {
    return NextResponse.json({ error: "Quoted price is invalid." }, { status: 400 });
  }

  const { data, error } = await access.supabase.rpc("mobile_create_service_call_atomic", {
    ...
    p_vehicle_year: body.vehicleYear == null ? null : Math.trunc(Number(body.vehicleYear)),
  ...
  if (error) {
    const conflict = error.code === "23P01" || error.code === "40001" || /available|conflict/i.test(error.message);
    return NextResponse.json({ error: error.message }, { status: error.code === "42501" ? 403 : conflict ? 409 : 400 });
  }
