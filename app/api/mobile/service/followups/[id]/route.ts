...
type Body = {
  status?: "quoted" | "converted" | "dismissed";
...
export async function PATCH(
  ...
  if (
    !id ||
    !status ||
    !["quoted", "converted", "dismissed"].includes(status) ||
    !operationKey
  ) {
    return NextResponse.json(
      { error: "Follow-up status and operation key are required." },
      { status: 400 },
    );
  }
  ...
  const { data, error } = await rpc(
    "mobile_update_service_followup_status_atomic",
    {
      ...
      p_followup_id: id,
      p_status: status,
      p_converted_work_order_id: body?.convertedWorkOrderId?.trim() || null,
