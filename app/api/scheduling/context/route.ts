...
export async function GET() {
  ...
  if (meErr || !me || !me.shop_id) {
    return NextResponse.json({ error: "Profile not found or missing shop" }, { status: 403 });
  }

  const canEditAll = ADMIN_ROLES.has(String(me.role ?? "").toLowerCase());

  let users: Array<{ id: string; full_name: string | null; role: string | null; shop_id: string | null }> = [];
  if (canEditAll) {
