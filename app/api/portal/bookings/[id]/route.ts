...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (!isStaff && !isCustomerOwner) {
    // RLS would also block, but give a friendly message
    return bad("Not allowed", 403);
  }

  // 4) Validate status transition
  const curr = booking.status as PatchBody["status"];
  const allowedTransitions: Record<NonNullable<PatchBody["status"]>, NonNullable<PatchBody["status"]>[]> = {
  ...
  // 7) Apply update (RLS will enforce shop/customer scope)
