...
import { syncAppointmentPartsPreparation } from "@/features/operations/server/syncAppointmentPartsPreparation";
...
function authorizeInternalRequest(
  ...
  return requireInternalApiSecret({
    ...
    routeLabel: "internal/appointment-parts-preparation",
...
export async function GET(request: Request) {
  ...
  const summaries: Awaited<ReturnType<typeof syncAppointmentPartsPreparation>>[] =
    [];
  ...
  for (let index = 0; index < shopIds.length; index += CONCURRENCY) {
    ...
    const results = await Promise.allSettled(
      batch.map((shopId) =>
        syncAppointmentPartsPreparation({ supabase, shopId, now }),
      ),
    );
      ...
      warnings.push({
        ...
        error:
          result.reason instanceof Error
            ...
            : "Unknown appointment parts preparation error",
      });
    });
  }
