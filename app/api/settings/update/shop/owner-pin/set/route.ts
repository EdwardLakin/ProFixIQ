...
export async function POST(req: Request) {
  try {
    ...
    if (!shopId || !newPin) {
      return NextResponse.json({ error: "shopId and newPin are required" }, { status: 400 });
    }
    ...
    if (shop.owner_id !== user.id) {
      return NextResponse.json({ error: "Only the shop owner can set the PIN" }, { status: 403 });
    }
    ...
    if (shop.owner_pin_hash) {
      ...
      if (!ok) return NextResponse.json({ error: "Invalid current PIN" }, { status: 401 });
    }
