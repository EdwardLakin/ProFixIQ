...
const RPC_FAILURES = {
  DEFERRED_RECOMMENDATION_SERVICE_ROLE_REQUIRED: {
    status: 403,
    message: "You do not have permission to decline this recommendation.",
  },
...
function rpcFailureResponse(error: DatabaseErrorLike) {
  const safe = toSafeDatabaseError(error, {
    context: "work-orders/deferred-history/decline",
    fallback: "Unable to decline this recommendation.",
...
export async function POST(
  ...
  if (
    ...
    idempotencyKeyResult.data.toLowerCase() !== bodyResult.data.actionId
  ) {
    return NextResponse.json(
      { ok: false, error: "The idempotency key must match the action id." },
      { status: 400 },
    );
  }

  const admin = createAdminSupabase();
  const { data, error } = await admin.rpc("decline_deferred_recommendation", {
