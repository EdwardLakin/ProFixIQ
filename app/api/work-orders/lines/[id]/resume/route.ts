...
export async function POST(req: NextRequest) {
  ...
  const { data: line, error: lineErr } = await supabase
    ...
    .select(
      "id, status, approval_state, punchable, assigned_tech_id, shop_id, punched_in_at, punched_out_at",
    )
    .eq("id", id)
    .maybeSingle();
  ...
  if (!openShift) {
    return NextResponse.json(
      { error: "Cannot resume job without an active daily punch. Clock in first." },
      { status: 409 },
    );
  }
  ...
  const shouldResetPunchClock = status === "on_hold" || Boolean(line.punched_out_at);
