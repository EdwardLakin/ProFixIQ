...
export async function GET() {
  ...
  if (expiredCertifications > 0) add("certifications", { id: "cert-expired", type: "cert_expired", severity: "blocking", title: "Expired certifications", description: `${expiredCertifications} certification${expiredCertifications > 1 ? "s are" : " is"} expired.`, count: expiredCertifications, href: "/dashboard/workforce/people?action=cert_expired" });
  if (expiringCertifications > 0) add("certifications", { id: "cert-expiring", type: "cert_expiring", severity: "warning", title: "Certifications expiring soon", description: `${expiringCertifications} certification${expiringCertifications > 1 ? "s" : ""} expiring in 30 days.`, count: expiringCertifications, href: "/dashboard/workforce/people?action=cert_expiring" });
  ...
  if (unassignedJobs > 0) add("operations", {
    id: "unassigned-jobs", type: "unassigned_jobs", severity: "warning", title: "Unassigned active jobs", description: `${unassignedJobs} active job line${unassignedJobs > 1 ? "s" : ""} without technician assignment.`, count: unassignedJobs, href: "/dashboard/work-orders",
    // TODO(workforce-phase-2b): add /dashboard/work-orders?assignment=unassigned&status=active when work-order filter support is implemented.
  });
