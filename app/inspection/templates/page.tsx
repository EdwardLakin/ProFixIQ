...
type InspectionTemplate = Database['public']['Tables']['inspection_templates']['Row'];
...
export default function InspectionTemplatesPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold text-white mb-4">Shared Inspection Templates</h1>
            ...
            >
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="secondary" onClick={() => handleEdit(template.id)}>
                    Edit
                  </Button>
              ...
              <p className="text-xs text-gray-400">
                Vehicle: {template.vehicle_type || 'N/A'} | Tags: {template.tags || 'None'} | Owner: {template.user_id}
              </p>
              <p className="text-xs text-gray-500 mt-1">
                Created: {template.created_at ? new Date(template.created_at).toLocaleString() : 'N/A'}
              </p>
            </div>
          ))}
          {filtered.length === 0 && <p className="text-white">No templates found.</p>}
        </div>
      )}
    </div>
  );
