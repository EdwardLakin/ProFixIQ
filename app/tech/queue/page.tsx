...
import { getQueuedJobsForTech, type TechQueueJob } from "@work-orders/lib/work-orders/getQueuedJobsForTech";
...
export default function TechQueuePage() {
  ...
  async function fetchJobsAndProfile() {
    ...
    setTech(profile ? { id: profile.id, full_name: profile.full_name } : null);

    // Returns TechQueueJob[] already normalized
    const result = await getQueuedJobsForTech({ techId: profile?.id ?? null });
    setJobs(result);
    setLoading(false);
  }
