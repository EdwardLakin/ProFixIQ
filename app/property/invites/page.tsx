...
export default async function PropertyInvitesPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="min-h-screen bg-[var(--theme-gradient-panel)] p-6 text-[color:var(--theme-text-primary)]"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="metal-card rounded-3xl p-6"><h1 className="text-3xl font-semibold">Property Portal Invites</h1><p className="mt-2 text-sm text-[color:var(--theme-text-secondary)]">Create internal invite records for property portal access scopes.</p><p className="mt-2 text-xs text-[color:var(--theme-text-secondary)]">Invite created. Email sending and token acceptance will be wired in the next phase.</p>{sv(params.error)&&<p className="mt-2 text-rose-300">{sv(params.error)}</p>}</section>
    ...
    <article className="metal-card rounded-3xl p-6"><h2 className="text-lg font-semibold">Existing invite records</h2><div className="mt-4 space-y-3">{inviteRows.length===0?<p className="text-sm text-[color:var(--theme-text-secondary)]">No property portal invites found for this shop.</p>:inviteRows.map((invite)=><div key={invite.id} className="rounded-2xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] p-3 text-sm"><div className="flex justify-between"><span>{invite.invited_email}</span><span>{invite.status}</span></div><div className="text-xs text-[color:var(--theme-text-secondary)]">Name: {invite.invited_name||"—"}<br/>Role: {invite.role}<br/>Scope: {scopeLabel(invite, portfolioMap, propertyMap, unitMap)}<br/>Link: Link not available. Create a new invite to generate a one-time link.<br/>Expires: {invite.expires_at?new Date(invite.expires_at).toLocaleString():"—"}<br/>Created: {invite.created_at?new Date(invite.created_at).toLocaleString():"—"}<br/>Accepted: {invite.accepted_at?new Date(invite.accepted_at).toLocaleString():"—"}</div></div>)}</div></article></section>
  </div></main>;
}
