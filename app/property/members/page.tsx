...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="min-h-screen bg-[var(--theme-gradient-panel)] p-6 text-[color:var(--theme-text-primary)]"><div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="metal-card rounded-3xl p-6"><h1 className="text-3xl font-semibold">Property Members</h1><p className="mt-2 text-sm text-[color:var(--theme-text-secondary)]">Manage property-scoped access for existing users.</p><p className="mt-2 text-xs text-[color:var(--theme-text-secondary)]">This does not send invites or create new auth users yet.</p>{sv(params.status)==="member-created"&&<p className="mt-2 text-emerald-300">Property member created.</p>}{sv(params.status)==="member-exists"&&<p className="mt-2 text-amber-300">Member already exists for that role and scope.</p>}{sv(params.error)&&<p className="mt-2 text-rose-300">{sv(params.error)}</p>}</section>
    <section className="grid gap-6 lg:grid-cols-2"><article className="metal-card rounded-3xl p-6"><h2 className="text-lg font-semibold">Create property member</h2><form action={createMember} className="mt-4 space-y-3">
      <select name="user_id" required className="w-full rounded-xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] px-3 py-2"><option value="">Select user</option>{pRows.map((p)=><option key={p.id} value={p.id}>{label(p)} ({p.id})</option>)}</select>
      ...
      <select name="portfolio_id" className="w-full rounded-xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] px-3 py-2"><option value="">No portfolio scope</option>{poRows.map((x)=><option key={x.id} value={x.id}>{x.name??x.id}</option>)}</select>
      <select name="property_id" className="w-full rounded-xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] px-3 py-2"><option value="">No property scope</option>{prRows.map((x)=><option key={x.id} value={x.id}>{x.name??x.id}</option>)}</select>
      <select name="unit_id" className="w-full rounded-xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] px-3 py-2"><option value="">No unit scope</option>{uRows.map((x)=><option key={x.id} value={x.id}>{x.unit_label??x.id}</option>)}</select>
    ...
    <article className="metal-card rounded-3xl p-6"><h2 className="text-lg font-semibold">Existing members</h2><div className="mt-4 space-y-3">{mRows.length===0?<p className="text-sm text-[color:var(--theme-text-secondary)]">No property members found for this shop.</p>:mRows.map((m)=><div key={m.id} className="rounded-2xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] p-3 text-sm"><div className="flex justify-between"><span>{pMap.get(m.user_id)?label(pMap.get(m.user_id)!):m.user_id}</span><span>{m.role}</span></div><div className="text-xs text-[color:var(--theme-text-secondary)]">Portfolio: {m.portfolio_id?(poMap.get(m.portfolio_id)?.name??m.portfolio_id):"—"}<br/>Property: {m.property_id?(prMap.get(m.property_id)?.name??m.property_id):"—"}<br/>Unit: {m.unit_id?(uMap.get(m.unit_id)?.unit_label??m.unit_id):"—"}<br/>Created: {m.created_at?new Date(m.created_at).toLocaleString():"—"}</div></div>)}</div></article></section>
  </div></main>;
}
