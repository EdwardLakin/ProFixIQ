...
export default async function PropertyInspectionsPage() {
  ...
  return <main className="min-h-screen bg-[var(--theme-gradient-panel)] p-6 text-[color:var(--theme-text-primary)]"><div className="mx-auto max-w-6xl rounded-3xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] p-6"><div className="mb-4 flex items-center justify-between"><div><h1 className="text-2xl font-semibold">Property inspections</h1><p className="text-sm text-[color:var(--theme-text-secondary)]">Internal property-maintenance inspections only.</p></div><div className="flex gap-2"><Link href="/property" className="rounded-full border px-3 py-1 text-xs">Back</Link><Link href="/property/inspections/new?type=move_in" className="rounded-full border border-[color:var(--accent-copper)]/70 bg-[color:var(--accent-copper)]/20 px-3 py-1 text-xs">New property inspection</Link></div></div>{!(inspections??[]).length ? <div className="rounded-2xl border border-dashed border-[color:var(--metal-border-soft)] p-6 text-sm text-[color:var(--theme-text-secondary)]">No property inspections yet.</div> : <div className="space-y-3">{(inspections??[]).map((x)=><article key={x.id} className="rounded-2xl border border-[color:var(--metal-border-soft)] bg-[color:var(--theme-surface-inset)] p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="text-sm font-semibold text-[color:var(--theme-text-primary)]">{x.inspection_type}</div><div className="mt-1 text-xs text-[color:var(--theme-text-secondary)]">{properties?.find((p)=>p.id===x.property_id)?.name ?? "Unknown property"}{x.unit_id ? ` · ${units?.find((u)=>u.id===x.unit_id)?.unit_label ?? "Unknown unit"}` : ""}</div><p className="mt-2 text-xs text-[color:var(--theme-text-secondary)]">{x.summary || "No summary provided."}</p></div><div className="text-right text-xs text-[color:var(--theme-text-secondary)]"><div>Status: {x.status}</div><div>Completed: {x.completed_at ? new Date(x.completed_at).toLocaleString() : "—"}</div><div>Created: {new Date(x.created_at).toLocaleString()}</div></div></div><div className="mt-2"><Link href={`/property/inspections/${x.id}`} className="text-xs underline">View detail →</Link></div></article>)}</div>}</div></main>;
}
