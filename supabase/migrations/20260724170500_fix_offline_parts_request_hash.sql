...
-- Re-apply the materializer with pgcrypto available in the function search path.
-- Production had the function locked to public only, so digest(...) was not found.
...
declare
  ...
  v_payload_hash text := encode(digest(v_payload::text, 'sha256'::text), 'hex');
  v_result jsonb;
begin
