...
-- Restore the first-write path for the canonical inspection writer. A
-- SELECT ... INTO with no canonical row clears v_server_revision to NULL.
-- Return the inserted row's revision so the following compare-and-swap uses
-- revision zero instead of comparing sync_revision to NULL.
