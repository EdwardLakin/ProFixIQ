...
-- Reinstall the legacy implementation with every non-catalog dependency
-- qualified. The old function relied on a public search path for pgcrypto and
-- also called a nonexistent jsonb_object_length helper, so moving it to a
-- private core exposed two latent clean-replay failures.
...
begin
  if auth.uid() is not null and auth.uid() <> p_actor_user_id then
    raise exception using
      errcode = 'P0001',
      message = 'Authenticated actor does not match the draft actor.';
  end if;
  ...
  if found then
    if v_existing.action_type <> 'materialize_work_order_draft'
       or v_existing.payload_hash <> v_payload_hash then
      raise exception using
        errcode = 'P0001',
        message = 'IDEMPOTENCY_KEY_REUSE: operation key belongs to different draft data.';
    end if;
  ...
  if not found
     or v_role not in (
       ...
       'lead_hand', 'lead hand', 'leadhand', 'lead', 'foreman'
     ) then
    raise exception using
      errcode = 'P0001',
  ...
  if not exists (
    ...
    raise exception using
      errcode = 'P0001',
      message = 'Vehicle not found for this customer and shop.';
  end if;
  if jsonb_typeof(v_payload -> 'lines') <> 'array'
    ...
    raise exception using
      errcode = 'P0001',
      message = 'Draft requires between 1 and 50 job lines.';
  end if;
  ...
  from public.create_work_order_with_custom_id(
    ...
    p_priority => greatest(
      1,
      least(5, coalesce((v_payload ->> 'priority')::integer, 3))
    ),
  ...
  loop
    ...
    if v_temp_id is null or v_line_map ? v_temp_id then
      raise exception using
        errcode = 'P0001',
        message = 'Every draft line requires a unique temporary ID.';
    end if;
    if coalesce(v_line ->> 'lineType', 'job') not in ('job', 'info') then
      raise exception using
        errcode = 'P0001',
        message = 'Unsupported draft line type.';
    end if;
    if nullif(trim(v_line ->> 'complaint'), '') is null then
      raise exception using
        errcode = 'P0001',
        message = 'Every draft line requires a description.';
    end if;
    if nullif(v_line ->> 'laborTime', '') is not null
       and (
         (v_line ->> 'laborTime')::numeric < 0
         or (v_line ->> 'laborTime')::numeric > 1000
       ) then
      raise exception using
        errcode = 'P0001',
        message = 'Draft line labor time is outside the allowed range.';
    end if;
    ...
    insert into public.work_order_lines(
      ...
      case
        when coalesce(v_line ->> 'lineType', 'job') = 'info'
          then nullif(trim(v_line ->> 'complaint'), '')
        else null
      end,
      nullif(trim(v_line ->> 'notes'), ''),
      case
        when coalesce(v_line ->> 'lineType', 'job') = 'info' then null
        when v_line ->> 'jobType' in (
          'diagnosis', 'inspection', 'maintenance', 'repair'
        ) then v_line ->> 'jobType'
        else 'diagnosis'
      end,
...
exception
  when unique_violation then
    ...
    where receipt.shop_id = p_shop_id
      and receipt.operation_key = p_operation_key;
    if found
       and v_existing.action_type = 'materialize_work_order_draft'
