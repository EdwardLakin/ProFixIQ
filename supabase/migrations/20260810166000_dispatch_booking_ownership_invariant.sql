...
-- Persist Dispatch ownership directly on the booking row. This makes the
-- handoff a table invariant instead of depending on trigger ordering.
alter table public.bookings
  add column if not exists dispatch_owner_visit_id uuid,
  add column if not exists dispatch_locked_starts_at timestamptz,
  add column if not exists dispatch_locked_ends_at timestamptz,
  add column if not exists dispatch_locked_status text;

-- If this migration is replayed over data already dispatched by an earlier
-- branch deployment, lock the booking at its current durable state.
update public.bookings b
set dispatch_owner_visit_id = sv.id,
    dispatch_locked_starts_at = b.starts_at,
    dispatch_locked_ends_at = b.ends_at,
    dispatch_locked_status = b.status
from public.service_visits sv
where sv.booking_id = b.id
  and sv.dispatched_at is not null
  and b.dispatch_owner_visit_id is null;

alter table public.bookings
  drop constraint if exists bookings_dispatch_owned_state_ck;

alter table public.bookings
  add constraint bookings_dispatch_owned_state_ck
  check (
    dispatch_owner_visit_id is null
    or (
      starts_at is not distinct from dispatch_locked_starts_at
      and ends_at is not distinct from dispatch_locked_ends_at
      and status is not distinct from dispatch_locked_status
    )
  );

create index if not exists bookings_dispatch_owner_visit_idx
  on public.bookings(dispatch_owner_visit_id)
  where dispatch_owner_visit_id is not null;

-- Stamp the durable booking lock at the exact scheduled -> dispatched handoff.
create or replace function public.sync_dispatch_booking_ownership_lock()
returns trigger
language plpgsql
security definer
...
begin
  if new.booking_id is null
     ...
     or old.dispatched_at is not null then
    return new;
  end if;

  update public.bookings
  set dispatch_owner_visit_id = new.id,
      dispatch_locked_starts_at = starts_at,
      dispatch_locked_ends_at = ends_at,
      dispatch_locked_status = status,
      updated_at = now()
...
revoke all on function public.sync_dispatch_booking_ownership_lock()
  from public, anon, authenticated, service_role;

drop trigger if exists service_visits_sync_dispatch_booking_ownership
  on public.service_visits;
create trigger service_visits_sync_dispatch_booking_ownership
after update of dispatched_at
on public.service_visits
for each row execute function public.sync_dispatch_booking_ownership_lock();

-- Terminal Dispatch outcomes are the only lifecycle writes allowed after
-- ownership handoff. Update the locked status in the same statement so the
-- table-level invariant remains satisfied.
...
begin
  ...
  set status = new.status,
      dispatch_locked_status = case
        when dispatch_owner_visit_id = new.id then new.status
        else dispatch_locked_status
      end,
...
-- Canonical Dispatch is allowed to move a dispatched visit before travel starts.
-- Move the booking and its persisted lock snapshot atomically; legacy booking
-- writers do not know these lock columns and therefore cannot move the row.
...
begin
  ...
  if v_visit.booking_id is not null then
    ...
    set starts_at = p_starts_at,
        ends_at = p_ends_at,
        dispatch_locked_starts_at = case
          when dispatch_owner_visit_id = v_visit.id then p_starts_at
          else dispatch_locked_starts_at
        end,
        dispatch_locked_ends_at = case
          when dispatch_owner_visit_id = v_visit.id then p_ends_at
          else dispatch_locked_ends_at
        end,
        updated_at = now(),
        lifecycle_metadata = coalesce(lifecycle_metadata, '{}'::jsonb)
