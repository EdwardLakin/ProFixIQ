-- Allow assigned technicians to document active approved work.
--
-- Customer authorization locks the commercial scope of a job, but the mobile
-- technician workspace deliberately exposes execution notes after approval.
-- The previous RPC rejected those notes at the database boundary. Completed
-- lines, shop scope, assignment, optimistic versioning, and idempotency remain
-- enforced by the function below.
...
begin
  ...
  if p_action_type = 'update_work_order_line_notes' then
    -- Technician notes document execution after customer authorization.
    -- Assignment and completed-line guards above remain authoritative.
...
comment on function public.apply_offline_line_mutation_atomic(uuid,uuid,text,text,uuid,jsonb)
is 'Applies retry-safe technician notes or story drafts to active, assigned work-order lines.';
