...
-- Reconcile the canonical AI recommendation/preview substrate into the tracked
-- migration chain. The live project inherited these objects from older remote
-- schema history, while clean installs did not. This remains recommendation-
-- and preview-first and does not enable autonomous action execution.
...
create policy "service-role-manage-ai-evidence-snapshots"
  on public.ai_evidence_snapshots to service_role
...
create policy "service-role-manage-ai-recommendations"
  on public.ai_recommendations to service_role
  using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');
...
create policy "service-role-manage-ai-action-previews"
  on public.ai_action_previews to service_role
  using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');
...
create policy "service-role-manage-ai-action-approvals"
  on public.ai_action_approvals to service_role
  using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');
...
create policy "service-role-manage-ai-action-events"
  on public.ai_action_events to service_role
  using (auth.role() = 'service_role')
  with check (auth.role() = 'service_role');

-- Authenticated tenant-safe review-layer access. Evidence and events remain
-- append-only to browser users; mutable recommendation and preview records use
-- the existing shop boundary helper.
do $$ begin
  create policy ai_evidence_snapshots_shop_select on public.ai_evidence_snapshots
    for select to authenticated using (is_shop_member_v2(shop_id));
exception when duplicate_object then null; end $$;

do $$ begin
  create policy ai_evidence_snapshots_shop_insert on public.ai_evidence_snapshots
    for insert to authenticated with check (is_shop_member_v2(shop_id));
exception when duplicate_object then null; end $$;
...
do $$ begin
  create policy ai_recommendations_shop_update on public.ai_recommendations
    for update to authenticated using (is_shop_member_v2(shop_id))
    with check (is_shop_member_v2(shop_id));
exception when duplicate_object then null; end $$;
