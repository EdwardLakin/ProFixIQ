...
create policy work_order_correction_sessions_shop_select
  ...
  using (
    ...
    or exists (
      ...
      where p.id = auth.uid() and p.shop_id = work_order_correction_sessions.shop_id
    )
  );
...
grant execute on function public.work_order_financial_lock_state(uuid, uuid) to authenticated, service_role;
...
set search_path = public
as $$
  select coalesce((public.work_order_financial_lock_state(p_shop_id, p_work_order_id) ->> 'locked')::boolean, false);
$$;

revoke all on function public.work_order_is_financially_locked(uuid, uuid) from public;
grant execute on function public.work_order_is_financially_locked(uuid, uuid) to authenticated, service_role;
...
begin
  ...
  select iv.id into v_invoice_version_id
  from public.invoice_versions iv
  where iv.shop_id = p_shop_id
    and iv.work_order_id = p_work_order_id
    and iv.lifecycle_status <> 'draft'
  ...
  from public.work_order_correction_sessions cs
  where cs.shop_id = p_shop_id and cs.operation_key = p_operation_key;
  if found then return v_session; end if;

  if exists (
    select 1 from public.work_order_correction_sessions cs
    where cs.work_order_id = p_work_order_id and cs.status = 'open'
  ) then
  ...
  insert into public.work_order_correction_sessions(
    shop_id, work_order_id, invoice_version_id, operation_key, reason, scope,
    opened_by, metadata
  ) values (
    p_shop_id, p_work_order_id, v_invoice_version_id, p_operation_key,
    trim(p_reason), v_scope, p_actor_user_id, coalesce(p_metadata, '{}'::jsonb)
  ) returning * into v_session;

  insert into public.financial_domain_outbox(
    shop_id, aggregate_type, aggregate_id, event_type, dedupe_key, payload
  ) values (
    p_shop_id, 'work_order_correction', v_session.id, 'work_order.correction_opened',
...
begin
  select * into v_session
  from public.work_order_correction_sessions cs
  where cs.id = p_correction_session_id and cs.shop_id = p_shop_id
  for update;
  if not found then
  ...
  set status = 'closed', closed_by = p_actor_user_id, closed_at = now(),
  ...
  insert into public.financial_domain_outbox(
    ...
    p_shop_id, 'work_order_correction', v_session.id, 'work_order.correction_closed',
...
revoke all on function public.open_work_order_correction_session(uuid,uuid,uuid,text,text,text,jsonb) from public, authenticated;
revoke all on function public.close_work_order_correction_session(uuid,uuid,uuid,jsonb) from public, authenticated;
grant execute on function public.open_work_order_correction_session(uuid,uuid,uuid,text,text,text,jsonb) to service_role;
grant execute on function public.close_work_order_correction_session(uuid,uuid,uuid,jsonb) to service_role;
...
begin
  if current_setting('profixiq.financial_mutation_bypass', true) = 'on' then
  ...
  if v_work_order_id is null and nullif(v_row ->> 'work_order_line_id', '') is not null then
    ...
    from public.work_order_lines wol where wol.id = v_line_id;
  end if;
  ...
  if v_work_order_id is null and nullif(v_row ->> 'work_order_part_id', '') is not null then
    ...
    select wop.work_order_id, coalesce(v_shop_id, wop.shop_id)
      into v_work_order_id, v_shop_id
    from public.work_order_parts wop where wop.id = v_parent_part_id;
  end if;

  if v_shop_id is not null and v_work_order_id is not null
     and public.work_order_is_financially_locked(v_shop_id, v_work_order_id) then
    raise exception using
      errcode = 'P0001',
      message = 'WORK_ORDER_FINANCIALLY_LOCKED',
      detail = format('Ordinary %s on %s is blocked after invoice finalization for work order %s', tg_op, tg_table_name, v_work_order_id),
...
declare
  ...
  v_new_has_history boolean;
  v_old_source jsonb;
  v_new_source jsonb;
  v_allowed_keys text[] := array[
    'updated_at','invoice_total','payment_status','outstanding_balance','paid_at'
  ];
begin
  if current_setting('profixiq.financial_mutation_bypass', true) = 'on' then return new; end if;

  v_old_locked := public.work_order_is_financially_locked(old.shop_id, old.id);
  v_new_has_history := coalesce((public.work_order_financial_lock_state(new.shop_id, new.id) ->> 'has_financial_history')::boolean, false);

  if not v_old_locked then
    return new;
  end if;
  ...
  if v_old_source is distinct from v_new_source then
    raise exception using
      errcode = 'P0001',
      message = 'WORK_ORDER_FINANCIALLY_LOCKED',
      detail = format('Operational work-order fields cannot change after invoice finalization for work order %s', old.id),
      hint = 'Open an audited correction session before changing finalized work-order source data.';
  end if;
...
drop trigger if exists trg_guard_financially_locked_work_order on public.work_orders;
...
begin
  foreach v_table in array v_tables loop
    if to_regclass('public.' || v_table) is not null then
      execute format('drop trigger if exists trg_guard_financially_locked_%I on public.%I', v_table, v_table);
      execute format(
...
comment on function public.guard_financially_locked_work_order() is
  'Allows canonical financial rollup fields while rejecting operational work-order mutations after invoice finalization.';
