-- Restore the payroll/time foundation required by audited shift corrections and
-- later workforce administration migrations. Existing databases are validated
-- and left unchanged; clean bootstraps receive the canonical shop-scoped model.
...
begin
  ...
  if v_mode = 'existing' then
    ...
    if coalesce(array_length(v_missing, 1), 0) > 0 then
      raise exception using errcode = 'P0001',
        message = 'PARTIAL_PROFIXIQ_SCHEMA: payroll/time foundation tables are missing: '
...
create table if not exists public.shop_payroll_settings (
  ...
  cadence text not null default 'biweekly'
    check (cadence in ('weekly','biweekly','semimonthly')),
  week_starts_on smallint not null default 1 check (week_starts_on between 0 and 6),
  daily_overtime_after_minutes integer not null default 480
    check (daily_overtime_after_minutes >= 0),
  weekly_overtime_after_minutes integer not null default 2400
    check (weekly_overtime_after_minutes >= 0),
  suspicious_shift_minutes integer not null default 960
    check (suspicious_shift_minutes >= 60),
...
create policy shop_payroll_settings_shop_crud on public.shop_payroll_settings
