...
begin
  ...
  if v_profile.shop_id is null then
    raise exception 'copilot_shop_required' using errcode = '42501';
  end if;
...
declare
  ...
  v_next_seq bigint;
begin
  select e.id, e.session_id, e.event_seq
  ...
  select rs.last_event_seq + 1 into v_next_seq
  ...
  insert into copilot.repair_session_events (
    session_id, shop_id, technician_id, event_seq, event_type, occurred_at
  ) values (
    p_session_id, p_shop_id, p_technician_id, v_next_seq, p_event_type, p_occurred_at
  ) returning id into v_event_id;

  insert into copilot.repair_session_event_context (
    event_id, shop_id, technician_id, operation_id, origin, details, occurred_at
  ) values (
    v_event_id, p_shop_id, p_technician_id, p_operation_id, p_origin, p_details, p_occurred_at
  );
