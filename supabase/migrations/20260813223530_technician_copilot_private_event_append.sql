-- Technician CoPilot V2 phase 2: private authorized event append handler.
...
begin
  ...
  if not copilot.technician_is_assigned(
    p_auth_user_id, v_profile_id, v_shop_id, v_work_order_id, null
  ) then
  ...
  if coalesce((v_result ->> 'replayed')::boolean, false) = false
     and p_event_type = 'task.changed' then
