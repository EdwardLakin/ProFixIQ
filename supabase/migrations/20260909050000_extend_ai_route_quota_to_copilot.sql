...
-- Extend the durable AI route quota to cover the Technician CoPilot.
--
-- The CoPilot turn is the platform's highest-frequency AI call and runs two
-- model requests per turn, but it had no durable ceiling: the feature whitelist
-- was fixed at ('dtc_suggest', 'inspection_interpret') in three places — the
-- receipts table CHECK and the input validation inside both quota RPCs — so a
-- claim for any other feature was rejected as AI_ROUTE_QUOTA_INPUT_INVALID.
--
-- This widens all three consistently. The change is additive: every value that
-- was accepted before is still accepted, no existing caller's behavior changes,
-- and the RPC signatures, ownership and grants are untouched. The two function
-- bodies below are reproduced verbatim from
-- 20260725162838_harden_p0_005_ai_route_governance.sql with only the whitelist
-- widened.
...
declare
  v_constraint text;
begin
  -- The original CHECK is inline and system-named; discover it rather than
  -- guessing. Matching on the whitelist text cannot collide with the status
  ...
  where nsp.nspname = 'private'
    and rel.relname = 'ai_route_usage_receipts'
    and con.contype = 'c'
    and pg_catalog.pg_get_constraintdef(con.oid) like '%dtc_suggest%';

  if v_constraint is not null then
    execute format(
      'alter table private.ai_route_usage_receipts drop constraint %I',
      v_constraint
    );
  end if;
end
...
alter table private.ai_route_usage_receipts
  add constraint ai_route_usage_receipts_feature_check
  check (feature in ('dtc_suggest', 'inspection_interpret', 'technician_copilot_text'));
