...
revoke all privileges on table private.ai_usage_ledger from public, anon, authenticated, service_role;
...
begin
  ...
  insert into private.ai_usage_ledger (
    ...
    btrim(p_event_key), p_shop_id, p_user_id, btrim(p_feature), btrim(p_endpoint),
    coalesce(nullif(btrim(p_provider), ''), 'openai'), nullif(btrim(p_model), ''),
    coalesce(nullif(btrim(p_modality), ''), 'text'), btrim(p_rate_card_version),
    p_prompt_tokens, p_cached_prompt_tokens, p_completion_tokens, p_total_tokens,
    p_audio_input_tokens, p_audio_output_tokens, p_speech_characters,
    p_duration_seconds, p_estimated_cost_usd, coalesce(p_latency_ms, 0), p_status,
    nullif(btrim(p_error_code), ''), left(nullif(btrim(p_error_message), ''), 500),
    nullif(btrim(p_provider_request_id), ''), p_quota_receipt_id,
