...
const DOMAIN_FIELD_ALIASES: Record<OnboardingDomain, Record<string, string[]>> = {
  customers: {
    sourceCustomerId: ["customer id", "id", "external customer id", "customer_number", "custno", "cust no", "customer no", "customer #"],
  ...
  vehicles: {
    sourceVehicleId: ["vehicle id", "id", "external vehicle id", "vehicle_number", "vehicle-id", "unit id"],
    sourceCustomerId: ["customer id", "customer_number", "custno", "cust no", "customer #"],
    vin: ["vin", "vehicle vin", "vehicle_vin"],
    plate: ["plate", "license", "license plate", "license_plate", "plate #"],
  ...
  history: {
    ...
    sourceCustomerId: ["customer id", "customer_number", "custno", "cust no", "customer #"],
    sourceVehicleId: ["vehicle id", "vehicle_number", "vehicle-id"],
    vehicleVin: ["vin", "vehicle vin", "vehicle_vin"],
    openedDate: ["opened", "opened date", "open date", "date", "service date", "opened_date"],
    complaint: ["complaint", "description", "concern", "service text", "service performed", "concern/description"],
    cause: ["cause"],
  ...
  invoices: {
    invoiceNumber: ["invoice", "invoice number", "invoice id", "invoice_number", "inv no", "inv #"],
    sourceWorkOrderId: ["work order", "ro", "ro id", "repair order", "work order number", "work_order_number", "ro number"],
    sourceCustomerId: ["customer id", "customer_number", "custno", "cust no", "customer #"],
  ...
  parts: {
    sku: ["sku", "part sku"],
    partNumber: ["part number", "part #", "number", "part_number", "part no"],
    description: ["description", "name", "part"],
    vendorName: ["vendor", "supplier", "vendor name", "supplier name", "vendor_id", "vendor code"],
    cost: ["cost", "unit cost"],
    price: ["price", "list price", "sale price"],
    quantityOnHandRaw: ["qty", "quantity", "on hand", "quantity on hand", "on_hand", "qty on hand"],
  },
  vendors: {
    sourceVendorId: ["vendor id", "external vendor id", "vendor_number", "vendor code"],
  ...
  staff: {
    ...
    phone: ["phone", "mobile"],
    role: ["role", "job title", "position", "technician", "advisor", "username", "job-title"],
  },
  menu: {
