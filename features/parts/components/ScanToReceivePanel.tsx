...
export function ScanToReceivePanel(): JSX.Element {
  ...
  const startScan = async () => {
    ...
    const handler: QuaggaDetectedHandler = async (res) => {
      ...
      const { data: partRow } = await supabase
        .from("parts")
        .select("id, sku, part_number, normalized_part_key, source_intake_id")
        .eq("id", part_id)
        .maybeSingle();
  ...
  return (
    <section className="space-y-4" aria-labelledby="scan-to-receive-title">
      <div>
        <h2 id="scan-to-receive-title" className="text-lg font-semibold text-[color:var(--theme-text-primary)]">Scan to receive</h2>
        <p className="mt-1 text-sm text-[color:var(--theme-text-secondary)]">Fast intake by barcode into a PO or stock location.</p>
      </div>
