...
type VStock = { part_id: string; location_id: string; qty_available: number; qty_on_hand: number; qty_reserved: number };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (term) {
          // simple ILIKE on name / sku / category
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-3xl rounded-2xl bg-white p-4 shadow-xl">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded border px-2 py-1 text-sm text-neutral-600 hover:bg-neutral-50">
            Close
          </button>
        </div>
                ...
                {(parts ?? []).length === 0 ? (
                  ...
                  parts.map(p => (
                    <button
                      ...
                      className={`block w-full text-left px-3 py-2 border-b last:border-b-0 hover:bg-neutral-50 ${selectedPartId===p.id ? "bg-neutral-100" : ""}`}
                    >
                      ...
                      return (
                        <label key={s.location_id} className="flex items-center justify-between rounded border p-2">
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-neutral-500 truncate">{l?.name ?? String(s.location_id).slice(0,6)+"…"}</div>
                          </div>
              ...
              <div className="mt-4 flex justify-end">
                <button
                  ...
                  onClick={() => {
                    ...
                    onPick({ part_id: selectedPartId, location_id: (selectedLocId ?? undefined), qty });
                    // let parent refresh any lists
