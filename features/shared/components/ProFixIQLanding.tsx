      ...
      <header className="sticky top-0 z-40 border-b border-[color:var(--marketing-border)] bg-[rgba(247,249,252,0.92)] backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1400px] items-center justify-between px-5 sm:px-8">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <Link href="/request-demo" className="rounded-xl border border-[color:var(--marketing-border-strong)] bg-white px-4 py-2.5 text-sm font-bold text-[color:var(--marketing-ink)] transition hover:border-[color:var(--marketing-copper)]">Request demo access</Link>
          ...
          <button type="button" className="grid h-10 w-10 place-items-center rounded-lg border border-[color:var(--marketing-border)] bg-white lg:hidden" onClick={() => setMenuOpen((value) => !value)} aria-expanded={menuOpen} aria-label="Toggle navigation">{menuOpen ? <X size={19} /> : <Menu size={19} />}</button>
        </div>
        {menuOpen ? <div className="border-t border-[color:var(--marketing-border)] bg-white px-5 py-5 lg:hidden"><div className="flex flex-col gap-4">{[{ href: "#workflow", label: "Workflow" }, { href: "#product", label: "Product" }, { href: "#shop-boost", label: "Shop Boost" }, { href: "#pricing", label: "Pricing" }, { href: "/customer/sign-in", label: "Customer sign-in" }, { href: "/request-demo", label: "Request demo access" }, { href: sessionExists ? "/dashboard" : "/sign-in", label: sessionExists ? "Dashboard" : "Sign in" }].map((item) => <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)} className="text-sm font-bold">{item.label}</Link>)}<Link href="/compare-plans" className="mt-2 rounded-xl bg-[color:var(--marketing-copper)] px-4 py-3 text-center text-sm font-bold text-white">Start free trial</Link></div></div> : null}
      </header>
