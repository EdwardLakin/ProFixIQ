  ...
  const ControlsRow = (
    <>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        />
        <input
          className="min-w-48 flex-1 rounded border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm"
      ...
      {messages.map((m, i) => {
        ...
        const content =
          typeof m.content === "string"
            ? m.content
            : // if ever multimodal content lands here, show a simple text label
              (m.content as any[])
                .map((p: any) => ("text" in p ? p.text : "[image]"))
                .join("\n");
