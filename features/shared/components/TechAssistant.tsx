    ...
    <div className="space-y-6 text-white">
      ...
      <div className="rounded-lg border border-white/10 bg-black/40 backdrop-blur p-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <label className="rounded bg-neutral-800 px-3 py-2 text-sm hover:bg-neutral-700 cursor-pointer disabled:opacity-60 font-header">
            <input
              ref={fileRef}
              type="file"
              accept="image/*"
          ...
          <button
            className="rounded bg-red-600/80 px-3 py-2 text-sm text-white hover:bg-red-600 disabled:opacity-60 font-header"
            onClick={cancel}
            type="button"
            disabled={!sending}
          >
      ...
      <div className="rounded-lg border border-white/10 bg-black/40 backdrop-blur">
        <div ref={scrollRef} className="max-h-[60vh] overflow-y-auto p-4 space-y-3">
          {messages.map((m, i) => {
            ...
            return mine ? (
              <div key={i} className="flex justify-end">
                <div className={`${bubble} bg-orange-600 text-black font-header`}>{m.content}</div>
              </div>
            ) : (
                  ...
                  <div className="prose prose-invert prose-sm !text-neutral-200">
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        h3: ({ children }) => (
                          <h3 className="text-sm font-header text-white mt-2 mb-1">{children}</h3>
                        ),
