...
const In = z.object({
  ...
  type: z.enum(["maintenance","repair","diagnosis","inspection"]).default("inspection"),
  notes: z.string().nullable().optional()
});
export type CreateWorkOrderIn = z.infer<typeof In>;

const Out = z.object({ workOrderId: z.string().uuid() });
export type CreateWorkOrderOut = z.infer<typeof Out>;

export const toolCreateWorkOrder: ToolDef<CreateWorkOrderIn, CreateWorkOrderOut> = {
  ...
  async run(input, ctx) {
    ...
    if (error || !data) throw new Error(error?.message ?? "create_work_order failed");
