...
const cardBase =
  "rounded-2xl border border-[color:var(--theme-border-soft)] bg-[var(--theme-gradient-panel)] shadow-[var(--theme-shadow-medium)] backdrop-blur-xl";
  ...
  return (
    <section className={`space-y-6 p-4 sm:p-6 ${cardBase}`}>
      ...
      <div className="flex flex-col items-center text-center gap-2">
        ...
        <h2
          className="text-xl sm:text-2xl text-[color:var(--theme-text-primary)]"
        ...
        <div className={`mt-2 w-full max-w-3xl px-4 py-4 ${cardInner}`}>
          ...
          <p className="mt-2 text-[13px] sm:text-[14px] leading-relaxed text-[color:var(--theme-text-primary)] whitespace-pre-wrap">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Panel title="Top repairs by volume">
          <ul className="space-y-2 text-xs text-[color:var(--theme-text-primary)]">
            {safeMostCommon.slice(0, 5).map((repair) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="truncate text-[12px] font-medium text-[color:var(--theme-text-primary)]">
                    {repair.label}
                  </p>
            ...
            {safeMostCommon.length === 0 ? (
              <li className="text-[11px] text-[color:var(--theme-text-muted)]">
        ...
        <Panel title="Potential comeback risks">
          <ul className="space-y-2 text-xs text-[color:var(--theme-text-primary)]">
            {(comebackRisks ?? []).length > 0 ? (
              ...
              <li className="text-[11px] text-[color:var(--theme-text-muted)]">No obvious repeat issues detected yet.</li>
            )}
          </ul>
        </Panel>
          ...
          <ul className="space-y-2 text-xs text-[color:var(--theme-text-primary)]">
            {(fleetMetrics ?? []).length > 0 ? (
              fleetMetrics.map((metric) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-medium text-[color:var(--theme-text-primary)]">{metric.label}</p>
                    {metric.note ? <p className="text-[11px] text-[color:var(--theme-text-muted)]">{metric.note}</p> : null}
                  </div>
            ...
            (issuesDetected as ShopHealthIssue[]).map((iss) => (
              <li key={iss.key} className={`px-3 py-2 ${cardInner}`}>
                <div className="flex items-center justify-between gap-3">
                  <p className="text-[12px] font-semibold text-[color:var(--theme-text-primary)]">{iss.title}</p>
                  <span className="rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] px-2 py-0.5 text-[10px] text-[color:var(--theme-text-primary)]">
                    {iss.severity.toUpperCase()}
                  </span>
                </div>
                <p className="mt-1 text-[11px] text-[color:var(--theme-text-secondary)]">{iss.detail}</p>
                {iss.evidence ? (
                  <p className="mt-1 text-[10px] text-[color:var(--theme-text-muted)]">Evidence: {iss.evidence}</p>
                ) : null}
              </li>
            ))
          ) : (
          ...
          <ul className="space-y-2 text-xs text-[color:var(--theme-text-primary)]">
            {menuSuggestions.slice(0, 5).map((menu) => (
              <li key={menu.id} className={`px-3 py-2 ${cardInner}`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-[12px] font-medium text-[color:var(--theme-text-primary)]">{menu.name}</p>
                    {menu.targetVehicleYmm ? (
                      <p className="text-[11px] text-[color:var(--theme-text-muted)]">For {menu.targetVehicleYmm}</p>
                    ) : null}
                  </div>
                  <div className="text-right text-[11px] text-orange-200">
                ...
                <p className="mt-1 text-[11px] text-[color:var(--theme-text-secondary)]">{menu.description}</p>
              </li>
            ))}
          ...
          <ul className="space-y-2 text-xs text-[color:var(--theme-text-primary)]">
            {inspectionSuggestions.slice(0, 5).map((inspection) => (
              <li key={inspection.id} className={`px-3 py-2 ${cardInner}`}>
                <p className="text-[12px] font-medium text-[color:var(--theme-text-primary)]">{inspection.name}</p>
                <p className="text-[11px] text-[color:var(--theme-text-muted)]">
                  Best for: <span className="capitalize">{inspection.usageContext}</span> work
                </p>
                {inspection.note ? <p className="mt-1 text-[11px] text-[color:var(--theme-text-secondary)]">{inspection.note}</p> : null}
              </li>
            ))}
          ...
          {(recommendations ?? []).length > 0 ? (
            (recommendations as ShopHealthRecommendation[]).map((rec) => (
              <li key={rec.key} className={`px-3 py-2 ${cardInner}`}>
                <p className="text-[12px] font-semibold text-[color:var(--theme-text-primary)]">{rec.title}</p>
                <p className="mt-1 text-[11px] text-[color:var(--theme-text-secondary)]">{rec.why}</p>
                <ul className="mt-2 list-disc pl-4 text-[11px] text-[color:var(--theme-text-secondary)] space-y-1">
...
function MetricCard({ label, value, subValue }: MetricCardProps) {
  return (
    <div className={cardInner + " p-3"}>
      <p className="text-[11px] text-[color:var(--theme-text-muted)]">{label}</p>
      <p className="mt-1 text-lg font-semibold text-[color:var(--theme-text-primary)]">{value}</p>
      {subValue ? <p className="mt-0.5 text-[11px] text-[color:var(--theme-text-muted)]">{subValue}</p> : null}
    </div>
  );
}
...
function Panel({ title, children }: PanelProps) {
  return (
    <section className={cardBase + " p-3 sm:p-4"}>
      <h3 className="mb-2 text-sm font-semibold text-[color:var(--theme-text-primary)]">{title}</h3>
      {children}
    </section>
  );
}