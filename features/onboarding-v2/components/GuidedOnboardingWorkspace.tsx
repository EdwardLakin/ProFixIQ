...
export default function GuidedOnboardingWorkspace({ initialSessionId }: Props) {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-6 px-4 pb-10 pt-6 text-[color:var(--theme-text-primary)] sm:px-6 lg:px-8">
      <header className="rounded-[2rem] border border-[color:var(--theme-border-soft)] bg-[radial-gradient(circle_at_top_left,rgba(251,146,60,0.20),transparent_36%),linear-gradient(180deg,rgba(255,255,255,0.07),rgba(255,255,255,0.02))] p-6 shadow-[var(--theme-shadow-medium)] backdrop-blur-xl">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-orange-200/80">Setup control room</p>
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <h1 className="text-3xl font-semibold text-[color:var(--theme-text-primary)] sm:text-4xl">Guided Setup</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[color:var(--theme-text-secondary)]">
          ...
          {sessionId ? (
            <button
              type="button"
              className="rounded-full bg-orange-400 px-5 py-2.5 text-sm font-semibold text-[color:var(--theme-text-primary)] shadow-lg shadow-orange-500/20 transition hover:bg-orange-300 disabled:cursor-not-allowed disabled:opacity-60"
              disabled={isBusy}
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-5">
          {!detail ? (
            ...
            >
              <div className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-5">
                <p className="text-xs uppercase tracking-[0.16em] text-[color:var(--theme-text-secondary)]">Starting point</p>
                <h2 className="mt-2 text-2xl font-semibold text-[color:var(--theme-text-primary)]">Do you have an existing shop/system to import?</h2>
                <div className="mt-5 flex flex-wrap gap-3">
                  ...
                  <button
                    type="button"
                    disabled={isBusy}
                    className="rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] px-5 py-2.5 text-sm font-semibold text-[color:var(--theme-text-primary)] transition hover:border-emerald-300/60 hover:bg-emerald-300/10 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <OnboardingHighlightFrame title={activeStep.title} description={activeStep.shortDescription}>
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-4">
                  <p className="text-xs uppercase tracking-[0.16em] text-[color:var(--theme-text-secondary)]">Progress</p>
                  <p className="mt-2 text-3xl font-semibold text-[color:var(--theme-text-primary)]">{progress.percent}%</p>
                  <p className="mt-1 text-xs text-[color:var(--theme-text-secondary)]">{progress.completed} done · {progress.skipped} skipped · {progress.total} total</p>
                </div>
                <div className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-4 md:col-span-2">
                  <p className="text-xs uppercase tracking-[0.16em] text-[color:var(--theme-text-secondary)]">Current step</p>
                  <h2 className="mt-2 text-2xl font-semibold text-[color:var(--theme-text-primary)]">{activeStep.question}</h2>
                  <div className="mt-5 flex flex-wrap gap-3">
                    <button
                      type="button"
                      disabled={isBusy}
                    ...
                    <button
                      ...
                      className="rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] px-5 py-2.5 text-sm font-semibold text-[color:var(--theme-text-primary)] transition hover:border-yellow-300/60 hover:bg-yellow-300/10 disabled:cursor-not-allowed disabled:opacity-60"
            ...
            <OnboardingHighlightFrame title="Guided setup complete" description="There are no remaining guided setup steps.">
              <div className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-5 text-sm text-[color:var(--theme-text-secondary)]">Return to the dashboard when you are ready.</div>
            </OnboardingHighlightFrame>
          )}
        </section>

        <aside className="rounded-3xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-4 shadow-[var(--theme-shadow-medium)] backdrop-blur-xl">
          <div className="flex items-center justify-between gap-3 border-b border-[color:var(--theme-border-soft)] pb-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--theme-text-secondary)]">Step rail</p>
              <p className="mt-1 text-sm text-[color:var(--theme-text-secondary)]">Customer-first setup path</p>
            </div>
            <span className="rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)] px-2 py-1 text-xs text-[color:var(--theme-text-secondary)]">{progress.completed}/{progress.total}</span>
          </div>
          <ol className="mt-4 space-y-3">
            {GUIDED_ONBOARDING_STEPS.map((step) => {
              ...
              return (
                <li key={step.key} className={`rounded-2xl border p-3 ${isCurrent ? "border-orange-300/50 bg-orange-300/10" : "border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-subtle)]"}`}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-[color:var(--theme-text-primary)]">{step.order / 10}. {step.title}</p>
                      <p className="mt-1 text-xs leading-5 text-[color:var(--theme-text-secondary)]">{step.shortDescription}</p>
                    </div>
                    <span className="shrink-0 rounded-full border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] px-2 py-0.5 text-[0.65rem] text-[color:var(--theme-text-secondary)]">{stepStatusLabel(stored?.status ?? "not_started")}</span>
                  </div>
                  {sessionId ? (
                    <button
                      type="button"
