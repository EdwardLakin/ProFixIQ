...
export default function WorkforceOverviewClient() {
  ...
  if (error || !data) return <div className="rounded-xl border border-red-500/40 bg-red-950/30 p-5 text-sm text-red-100">{error ?? "Failed to load"} <button className="ml-3 underline disabled:cursor-not-allowed disabled:opacity-60" onClick={() => void load()} disabled={loading}>Retry</button></div>;
