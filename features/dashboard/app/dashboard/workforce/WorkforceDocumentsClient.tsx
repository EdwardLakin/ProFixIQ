  ...
  return <div className="space-y-5">
    ...
    {requirementsMode ? <section className="space-y-3 rounded-2xl border border-cyan-500/20 bg-cyan-950/10 p-4">
      ...
      {showOverrideForm ? <div className="space-y-2 rounded-xl border border-white/10 bg-black/30 p-3">
        <div className="text-sm font-semibold text-white">{editingOverrideId ? "Edit override" : "Create override"}</div><p className="text-xs text-neutral-400">System defaults apply unless an active shop override exists.</p><p className="text-xs text-neutral-400">Inactive overrides are ignored; defaults continue to apply.</p><p className="text-xs text-neutral-400">Overrides are disabled, not deleted.</p>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Workforce role" value={overrideForm.workforce_role} onChange={(e) => setOverrideForm((prev) => ({ ...prev, workforce_role: e.target.value }))} />
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Workforce category" value={overrideForm.workforce_category} onChange={(e) => setOverrideForm((prev) => ({ ...prev, workforce_category: e.target.value }))} />
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Document type" value={overrideForm.doc_type} onChange={(e) => setOverrideForm((prev) => ({ ...prev, doc_type: e.target.value }))} />
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Display label" value={overrideForm.label} onChange={(e) => setOverrideForm((prev) => ({ ...prev, label: e.target.value }))} />
          <label className="flex items-center gap-2 text-xs text-neutral-300"><input type="checkbox" checked={overrideForm.expires_required} onChange={(e) => setOverrideForm((prev) => ({ ...prev, expires_required: e.target.checked }))} />Expiration required</label>
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Expiration warning window (days)" value={overrideForm.expires_warning_days} onChange={(e) => setOverrideForm((prev) => ({ ...prev, expires_warning_days: e.target.value }))} />
          <label className="flex items-center gap-2 text-xs text-neutral-300"><input type="checkbox" checked={overrideForm.is_active} onChange={(e) => setOverrideForm((prev) => ({ ...prev, is_active: e.target.checked }))} />Active override</label>
        </div>
        ...
        {showAdvanced ? <div className="grid gap-2 sm:grid-cols-2">
          <label className="flex items-center gap-2 text-xs text-neutral-300"><input type="checkbox" checked={overrideForm.is_required} onChange={(e) => setOverrideForm((prev) => ({ ...prev, is_required: e.target.checked }))} />Required</label>
          ...
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Accepted statuses (comma-separated)" value={overrideForm.accept_statuses} onChange={(e) => setOverrideForm((prev) => ({ ...prev, accept_statuses: e.target.value }))} />
          <input className="rounded border border-white/15 bg-black/30 px-2 py-1 text-sm" placeholder="Review statuses (comma-separated)" value={overrideForm.review_statuses} onChange={(e) => setOverrideForm((prev) => ({ ...prev, review_statuses: e.target.value }))} />
        </div> : null}
      ...
      {requirements ? <>
        <section className="space-y-2"><h3 className="text-sm font-semibold text-neutral-100">Defaults</h3><p className="text-xs text-neutral-400">System defaults apply unless an active shop override exists.</p>{renderRequirementRows(requirements.defaults)}</section>
