...
export default function WorkforceDocumentsClient() {
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-5"><h1 className="text-2xl font-semibold text-[color:var(--theme-text-primary)]">Documents Command</h1><p className="mt-1 text-sm text-[color:var(--theme-text-secondary)]">Workforce readiness for document collection and compliance.</p><p className="mt-2 text-xs text-[color:var(--theme-text-secondary)]">Signed document access opens a short-lived secure link.</p><WorkforceQuickLinks roleScope="owner_admin" className="mt-3 flex flex-wrap gap-2" /><div className="mt-3 flex flex-wrap gap-2 text-xs"><Link href="/dashboard/workforce/people" className="rounded border border-[color:var(--theme-border-soft)] px-2 py-1 text-orange-300">Open People</Link><Link href="/dashboard/admin/employee-docs" className="rounded border border-[color:var(--theme-border-soft)] px-2 py-1 text-[color:var(--theme-text-secondary)]">Open Upload Console</Link><Link href={requirementsMode ? "/dashboard/workforce/documents" : "/dashboard/workforce/documents?mode=requirements"} className="rounded border border-cyan-500/40 px-2 py-1 text-cyan-300">{requirementsMode ? "Back to documents" : "Manage requirements"}</Link></div></div>
    ...
    {requirementsMode ? <section className="space-y-3 rounded-2xl border border-cyan-500/20 bg-cyan-950/10 p-4">
      ...
      {showOverrideForm ? <div className="space-y-2 rounded-xl border border-[color:var(--theme-border-soft)] bg-[color:var(--theme-surface-inset)] p-3">
        ...
        <div className="flex gap-2"><button disabled={savingOverride} className="rounded border border-emerald-500/40 px-3 py-1 text-sm text-emerald-200 disabled:opacity-50" onClick={() => void saveOverride()}>{savingOverride ? "Saving..." : editingOverrideId ? "Save changes" : "Create override"}</button><button className="rounded border border-[color:var(--theme-border-soft)] px-3 py-1 text-sm" onClick={() => { setShowOverrideForm(false); setEditingOverrideId(null); }}>Cancel</button></div>
      </div> : null}
