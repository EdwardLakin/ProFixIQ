// Technician CoPilot-owned OpenAI Realtime transcription transport.
// This intentionally does not change or wrap the existing inspection/dictation
// transport while the first voice collaborator slice is being proven.
...
export type TechnicianRealtimeVoiceState = "idle" | "connecting" | "listening" | "error";

type HandleTranscriptFn = (text: string) => void;

type TechnicianRealtimeVoiceOptions = {
  onStateChange?: (state: TechnicianRealtimeVoiceState) => void;
  onPulse?: () => void;
...
export function useTechnicianRealtimeVoice(
  ...
  opts?: TechnicianRealtimeVoiceOptions,
) {
  ...
  const setState = (state: TechnicianRealtimeVoiceState) => {
    ...
    try {
      ...
      ws.onmessage = (event) => {
        ...
        if (opts?.debug && type) {
          ...
          console.log("[TechnicianRealtimeVoice] event:", type);
        }
        ...
        if (type === "error") {
          ...
          // eslint-disable-next-line no-console
          console.error("[TechnicianRealtimeVoice] error", msgUnknown);
