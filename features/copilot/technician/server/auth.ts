...
import { isTechnicianCopilotTextEnabled } from "./featureGate";
...
export async function requireTechnicianCopilotAccess() {
  ...
  if (auth.error || !user) {
    throw new TechnicianCopilotAccessError(401, "unauthorized", "Authentication required.");
  }
  ...
  const enabled = await isTechnicianCopilotTextEnabled({
    supabase,
