...
const GENERIC_TECHNICIAN_NAMES = new Set([
  "technician",
  "assigned tech",
...
export function resolveTechnicianDisplayName(
  ...
  identifiableFallback: string | null | undefined,
): string | null {
  const projected = projectedName?.trim() || null;
  const fallback = identifiableFallback?.trim() || null;
  if (
    !projected ||
    GENERIC_TECHNICIAN_NAMES.has(projected.toLowerCase())
  ) {
