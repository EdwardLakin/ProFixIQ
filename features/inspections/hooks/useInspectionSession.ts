...
type SessionWithLineId = InspectionSession & {
  workOrderLineId?: string | null;
};

type StartSessionPayload = Partial<SessionWithLineId> & {
  axleConfig?: AxleLayoutConfig;
};
...
export default function useInspectionSession(initialSession?: Partial<SessionWithLineId>) {
  const [session, setSession] = useState<SessionWithLineId>(() => ({
    id: "",
    vehicleId: "",
    customerId: "",
    workOrderId: "",
  ...
  const startSession = (sessionData: StartSessionPayload) => {
  ...
  const setIsListening = (value: boolean) =>
    updateInspection({ isListening: value });
