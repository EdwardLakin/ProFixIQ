...
export default function RoleNavTech() {
  ...
  const sectionHeader = (title: string, open: boolean, toggle: () => void) => (
    <button onClick={toggle} className="w-full flex items-center justify-between px-2 py-2 text-orange-500 font-bold">
      <span>{title}</span>
  ...
  return (
    <nav className="w-full md:w-64 bg-neutral-900 p-4 text-white space-y-4">
      <div>
        ...
        {jobOpen && (
          <div className="pl-2 space-y-1">
            ...
            <Link href="/work-orders/view" className={linkClass('/work-orders/view')}>
      ...
      <div>
        {sectionHeader('Inspections', inspectionOpen, () => setInspectionOpen(!inspectionOpen))}
        {inspectionOpen && (
          <div className="pl-2 space-y-1">
      ...
      <div>
        {sectionHeader('Messaging', messagingOpen, () => setMessagingOpen(!messagingOpen))}
        {messagingOpen && (
          <div className="pl-2 space-y-1">
