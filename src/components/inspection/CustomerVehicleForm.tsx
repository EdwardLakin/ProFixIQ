...
export default function CustomerVehicleForm({ customer, vehicle, onCustomerChange, onVehicleChange }: Props) {
  return (
    <div className="w-full max-w-2xl mx-auto text-white space-y-6 mb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <input type="number" className="input" placeholder="Year" value={vehicle.year ?? ''} onChange={(e) => onVehicleChange('year', e.target.value)} />
        ...
        <input type="number" className="input" placeholder="Mileage" value={vehicle.mileage ?? ''} onChange={(e) => onVehicleChange('mileage', e.target.value)} />
