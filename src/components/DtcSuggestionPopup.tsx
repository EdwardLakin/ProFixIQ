...
export default function DtcSuggestionPopup({ jobId, vehicle, punchedInAt, onSave }: Props) {
      ...
      <div className="bg-white text-black p-6 rounded-lg max-w-lg w-full shadow-lg space-y-4">
        ...
        {result && (
          <div className="bg-gray-100 p-3 rounded text-sm space-y-2">
            <div><strong>Cause:</strong> {result.cause}</div>
            <div><strong>Correction:</strong> {result.correction}</div>
            <div><strong>Est. Labor Time:</strong> {result.estimatedLaborTime}</div>
