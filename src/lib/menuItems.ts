...
export async function searchMenuItems(keyword: string, vehicle?: { year?: string; make?: string; model?: string }) {
  let query = supabase.from('menu_items').select('*').ilike('complaint', `%${keyword}%`);
