...
const syncAppointmentPreparationsMock = vi.fn();
...
vi.mock("@/features/operations/server/syncAppointmentPreparations", () => ({
  syncAppointmentPreparations: syncAppointmentPreparationsMock,
}));

describe("GET /api/internal/appointment-preparations", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
    requireInternalApiSecretMock.mockReturnValue({ ok: true });
    createAdminSupabaseMock.mockReturnValue({});
    syncAppointmentPreparationsMock.mockResolvedValue({
      shopId: "shop_1",
      upcoming: 0,
      resolved: 0,
      errors: [],
  ...
  it("uses the shared guard with Vercel cron bearer authorization enabled", async () => {
    ...
    const { GET } = await import("../app/api/internal/appointment-preparations/route");
    const response = await GET(
      new Request("https://example.test/api/internal/appointment-preparations"),
    );
  ...
  it("syncs every shop and aggregates the summary", async () => {
    ...
    syncAppointmentPreparationsMock
      .mockResolvedValueOnce({ shopId: "shop_1", upcoming: 3, resolved: 1, errors: [] })
      .mockResolvedValueOnce({ shopId: "shop_2", upcoming: 0, resolved: 0, errors: [] });
    ...
    expect(response.status).toBe(200);
    expect(body).toMatchObject({ ok: true, checkedShops: 2, upcoming: 3, resolved: 1, warnings: [] });
    expect(syncAppointmentPreparationsMock).toHaveBeenCalledTimes(2);
  });

  it("reports a per-shop failure as a warning instead of failing the whole run", async () => {
    fetchAllShopIdsMock.mockResolvedValue(["shop_1", "shop_2"]);
    syncAppointmentPreparationsMock
      .mockResolvedValueOnce({ shopId: "shop_1", upcoming: 0, resolved: 0, errors: [] })
  ...
  it("returns 500 when the shop list cannot be loaded", async () => {
    ...
    expect(syncAppointmentPreparationsMock).not.toHaveBeenCalled();
  });
});
